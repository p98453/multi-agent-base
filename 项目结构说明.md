...
| **src/agents** | 多智能体核心（路由智能体 + 专家智能体 × 3 + LangGraph StateGraph 编排器） |
| **src/models** | LangChain ChatOpenAI 工厂（全局单例 LLM 实例） |
| **src/utils** | JSONL 结构化日志 |
| **backend/services/rag_service** | RAG 核心：LangChain RecursiveCharacterTextSplitter 分块、OpenAIEmbeddings 向量化、Chroma 存储与检索、LCEL 链生成答案 |
