...
```
Frontend (Streamlit)  ←→  Backend (FastAPI)  ←→  Agent System (LangGraph)
        ↓                        ↓                         ↓
    用户界面               RESTful API          LangGraph StateGraph + LangChain LCEL
                               ↓
                    MemoryStorage（内存告警历史）
                    RAGService（LangChain Chroma 向量库）
```
...
| **LangChain** | **LLM 调用、Prompt 模板、LCEL 链、RAG 组件** | **0.3+** |
| **LangGraph** | **多智能体 StateGraph 编排** | **0.2+** |
...
| 技术 | 用途 |
|------|------|
| LangChain ChatOpenAI + Qwen LLM（SiliconFlow API）| 多智能体告警分析 + RAG 答案生成 |
| LangChain OpenAIEmbeddings + Qwen3-Embedding-8B | RAG 文本向量化 |
| LangGraph StateGraph | 多智能体工作流编排 |
...
4. MultiAgentSystem.analyze() → LangGraph graph.ainvoke()
   ├─ route_node: RouterAgent.route() → 关键词 + 正则评分
   ├─ expert_node: ExpertAgent.analyze() → LCEL 链 (prompt | LLM | parser)
   └─ aggregate_node: 综合路由结果 + 专家分析 + 性能指标
...
3. RAGService.query_and_generate()
   ├─ Chroma.similarity_search_with_score()（余弦检索 top-k）
   ├─ 拼接 Context
...
1. 在 `src/agents/optimized_expert.py` 的 `prompt_templates` 字典中添加新专家的 `ChatPromptTemplate.from_messages()` 模板
...
**文档版本**: 3.0.0 (LangChain/LangGraph 重构版) | **更新**: 2026-02-25
